...
let font;
let font2;
let imgSize = 200; //הגדרת גדלי האובייקטים למעלה
let mouseXPos, mouseYPos; //הגדרת העכבר
let scatterParticles = []; //הגדרת העיגולים
let targetDate; //הגדרת טיימר
//הגדרת רקע- ציור הגריד מאחורה
const squareSizeCm = 4; // הגדרת כל גודל ריבוע בס"מ
const squareSizePx = squareSizeCm * 10; // הגדרת הכל ריבוע בפיקסלים

const canvasWidth = 595; //הגדרת הקנבס
const canvasHeight = 842;
const strokeWeightPx = 2;
//כל מה שלמעלה הם הגדרה של הערכים בקוד

function preload() {
  //טעינה של כל האובייקטים אל הפוסטר
...
function draw() {
  ...
  //פקודת ציור האלמנטים והטקסט
  drawImagesAndText();
}

function drawGrid() {
  //פונקציית גריד וכל הערכים שקשורים בעיצוב שלו
...
function mouseMoved() {
  // הגדרת הזזת העכבר בשביל ליצור פיזור של עיגולים לתחושה קסומה
  mouseXPos = mouseX;
  mouseYPos = mouseY;

  let particle = {
    // הגדרת פונקציית עיגולים כאשר העכבר זז העיגולים הופכים לשקופים
    x: mouseXPos,
    y: mouseYPos,
    alpha: 255,
  };
...
function drawImagesAndText() {
  //פונקציית ציור האלמנטים והטקסט
  imageMode(CENTER);

  textFont(font); // הגדרה של הפונטים בפוסטר
  fill(255);
  text("Unleash Your Potential", 22, 729); //צבע, טקסט ומיקום

  //הגדרת הטיימר
  let currentDate = new Date(); // הגדרת הזמן הנוכחי
  let timeRemaining = targetDate - currentDate; //הגדרת הזמן בפוסטר-הזמן המוגדר פחות הזמן העכשוי

  if (timeRemaining > 0) {
    // הגדרה של ימים,שעות,דקות ושניות
    ...
    textSize(34);
    textFont(font2);
    text(countdownString, 23, 767);
  }
  //textFont(font2);
  //  text("October 6, 2023", 35, 767);

  textFont(font);
  text("Ein Bokek beach", 23, 805);
  ...
  for (let i = scatterParticles.length - 1; i >= 4; i--) {
    // הגדרת העלמות העיגולים הלבנים והכפלתם לפי זמן מסויים
